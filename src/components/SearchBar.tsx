...
export function SearchBar({ bjdong: _bjdong, onSearch, loading, pending, loadingAth, hasResults, onAdd, onAddAll, onClearPending }: Props) {
  ...
  useEffect(() => {
    ...
    setSuggestions(filterApt(aptIndex, q))
    setActiveIdx(-1)
  }, [query, aptIndex])
  ...
  const SuggestList = ({ isMobile = false }: { isMobile?: boolean }) =>
          ...
          <div style={{ display: 'flex', gap: 6 }}>
            {pending.length > 1 && (
              <button onMouseDown={(e) => { e.preventDefault(); onAddAll() }}
                style={{ fontSize: 11, padding: '2px 8px', cursor: 'pointer', background: '#1a73e8', color: '#fff', border: 'none', borderRadius: 2 }}>
                전체 추가
              </button>
            )}
            <button onMouseDown={(e) => { e.preventDefault(); onClearPending() }}
              style={{ fontSize: 11, padding: '2px 6px', cursor: 'pointer', background: 'none', color: '#888', border: '1px solid #ccc', borderRadius: 2 }}>
              ✕
            </button>
          </div>
        </div>
          ...
          <div style={{ background: '#fff', border: '1px solid #ddd', borderTop: 'none' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', padding: '6px 10px', background: '#f5f5f5', fontSize: 12 }}>
              ...
              <div style={{ display: 'flex', gap: 6 }}>
                {pending.length > 1 && (
                  <button onMouseDown={(e) => { e.preventDefault(); onAddAll() }}
                    style={{ fontSize: 11, padding: '2px 8px', background: '#1a73e8', color: '#fff', border: 'none', borderRadius: 2 }}>전체 추가</button>
                )}
                <button onMouseDown={(e) => { e.preventDefault(); onClearPending() }}
                  style={{ fontSize: 11, padding: '2px 6px', background: 'none', color: '#888', border: '1px solid #ccc', borderRadius: 2 }}>✕</button>
              </div>
            </div>
            {pending.map(r => (
              <div key={`${r.aptName}__${r.dongCode}`}
